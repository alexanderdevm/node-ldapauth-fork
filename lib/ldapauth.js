...
LdapAuth.prototype._adminBind = function(callback) {
  ...
  var self = this;
  this._adminClient.bind(this.bindDN, this.bindCredentials, function(err) {
    if (err) {
...
LdapAuth.prototype._search = function(searchBase, options, callback) {
  ...
  self._adminBind(function(bindErr) {
    ...
    self._adminClient.search(searchBase, options, function(
      searchErr,
      searchResult
    ) {
      if (searchErr) {
...
LdapAuth.prototype._findUser = function(username, callback) {
  ...
  var searchFilter = self.opts.searchFilter.replace(
    /{{username}}/g,
...
LdapAuth.prototype.authenticate = function(username, password, callback) {
  ...
  self._findUser(username, function(findErr, user) {
    ...
    self._userClient.bind(user[self.opts.bindProperty], password, function(
      bindErr
    ) {
      if (bindErr) {
        self.log &&
          self.log.trace("ldap authenticate: bind error: %s", bindErr);
        return callback(bindErr);
      }
      ...
      self._getGroups(user, function(groupErr, userWithGroups) {
        if (groupErr) {
          self.log &&
            self.log.trace(
              "ldap authenticate: group search error %s",
              groupErr
            );
        ...
        if (self.opts.cache) {
          ...
          bcrypt.hash(md5Password, self._salt, function(err, hash) {
            if (err) {
              self.log &&
                self.log.trace(
                  "ldap authenticate: bcrypt error, not caching %s",
                  err
                );
            } else {
